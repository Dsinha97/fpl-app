...
/**
 * The ingestion pipeline's own log — last run per function, plus row counts.
 *
 * Sprint 33 moved this off `/status` and onto `/settings` as a third tab.
 * It was already not in NAV_GROUPS (Sprint 22 put it behind the account
 * menu, on the reasoning that a data-freshness page belongs beside account
 * settings rather than competing for a nav slot) — this finishes that move
 * rather than leaving it as a route reachable only from a dropdown.
 *
 * Mounted only while its tab is showing: it issues a count query per table.
 */
