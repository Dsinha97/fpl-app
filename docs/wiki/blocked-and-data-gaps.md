...
|---|---|---|
| ~~Team strength (0 for all 20 clubs)~~ **Custom FDR — no longer blocked (2026-09-02)** | The pre-season blanket "strength is zero" no longer holds: `strength_overall_home`/`_away` are populated for all 20 clubs (2–4 scale, three distinct home tiers), checked live 2026-09-02. A custom analytical FDR has data to work from. Not started, not blocked | [fpl-api-constraints.md](fpl-api-constraints.md) |
...
| Accuracy scoreboard panel (`/status`) | The data layer (`lib/prediction-accuracy.ts`) is done; a panel built on one gameweek's residual would invite reading it as a verdict on the model. **Condition restated 2026-09-02**: the bar is ≥2 *scored* gameweeks, not ≥2 archived ones. `player_prediction_archive` now holds 2 distinct events (GW2 and GW3, 1284 rows, checked live 2026-09-02) — but GW3's deadline hasn't passed, so only GW2 has both an archive and a result. Archiving happens *before* a deadline, so archive count runs one ahead of what can actually be scored | [data-pipeline.md](data-pipeline.md) |
