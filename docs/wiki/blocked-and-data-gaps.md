...
| League 314 rank-ordering (top-1k sample) | GW1 is scored and `teams.played`/`points` are real now, so the original "ties on 0" reason no longer applies — but `league_entries` for `league_id=314` has 0 rows as of 2026-08-27, meaning it has simply never been synced (it's the millions-strong "Overall" system league, not one of the owner's leagues `sync-league-picks` targets). Blocks only Sprint 10's top-1k sample — the exact mini-league slice is live, see [ownership-and-leagues.md](ownership-and-leagues.md) | [roadmap.md](../roadmap.md) |
