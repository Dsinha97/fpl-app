...
| DSI-179 | xDC excludes **forwards** from defensive-contribution points they score | M7 | None — measured with zero variance over 677 player-gameweeks (DEF 10, MID 12, FWD 12), and `scoring_rules` independently agrees. Gate is the existing backtest: forward bias improves, nothing else regresses. See [sprints/sprint-38.md](sprints/sprint-38.md) §4 |
...
| DSI-54 | Price-change prediction step 3 — **falls half run 2026-09-21; gate passed at tight budgets** | M7 | Walk-forward over 36 nights: the model beats the naive baseline at K=10 (p=0.0156) and K=20 (p=0.0039, survives Bonferroni), ties at K=80, and the two converge as K grows. Absolute recall is low at every budget, and **nothing is wired into the app** — that needs weight storage plus a nightly job. Rises still held to ~GW10–12 (60 positives is unresolvable, not negative). Harness `scripts/price-falls-walkforward.ts`; see [sprints/sprint-38.md](sprints/sprint-38.md) §3 |
