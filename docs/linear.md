...
| — | Ownership-scaled price threshold — **gated 2026-09-21, MIXED, not shipped** | M7 | No Linear issue yet. Classification F1 improves both directions (rise 25.6→29.9, fall 10.1→16.0) but the ranking gain fails Bonferroni (best p=0.0414 at K=80, needs <0.0125). Falls scale steeply (31,790/pct); **rises barely scale at all** (1,740/pct) — their problem is the level (~337k vs shipped 200k), not the shape. Harness `scripts/price-threshold-gate.ts`; see [sprints/sprint-38.md](sprints/sprint-38.md) §2c |
