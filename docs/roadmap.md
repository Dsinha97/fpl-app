...
| 10 | Ownership Intelligence | **Exact-slice built** 2026-08-21 — mini-league EO (`sync-league-picks`, `lib/ownership.ts`), rendered on `/leagues` (2026-08-30). Top-1k sample: **not blocked as an engineering matter** — a 2026-08-30 load test synced league 314 end to end (2000 rank-ordered entries, 30,000 picks, 0 failures in 47s; test data deleted after verification) — and **sampled for real 2026-09-10** (Sprint 36) — 2,000 entries and 30,795 picks, kept this time | [sprints/sprint-10.md](sprints/sprint-10.md), [sprints/sprint-29.md](sprints/sprint-29.md), [sprints/sprint-36.md](sprints/sprint-36.md) |
...
| 32 | Edge Function lockdown | **Built 2026-09-05, fully deployed and verified 2026-09-06** (Sprint 34; see sprint-32.md §5c) — every `/functions/v1/` endpoint was callable by anyone holding the publishable key, which is public by design. Split by caller: eight cron-only functions now require an `x-cron-secret` header minted by `invoke_sync` from Vault (`_shared/cron-auth.ts` — the inverse of Sprint 31's verdict on the publishable key, for the inverse reason: this secret has no public copy, so hiding it *is* the mechanism), and the two browser-invoked ones get `verifyUser` plus a per-user rate limit off a new `sync_runs.invoked_by`, with the limit a `game_settings` input derived from the busiest 10-minute window ever observed (27 calls) rather than a tuned constant. The scope doc named one `?force=1` self-gate bypass; grepping found **five**. `sync_runs`' public-read policy meant `invoked_by` would have published user ids, fixed with a column-level grant. CORS tightened off `*` for the browser-facing pair only and correctly labelled defence-in-depth; `verify_jwt` made explicit per function in `config.toml`. Deployment was an owner action in a fixed order (Vault secret, migration, then functions — the reverse silently 401s every cron job): steps 1-2 and 2 of 10 functions landed 2026-09-05, the remaining eight on 2026-09-06, when the verification gate was also run for the first time and caught `sync-manager` failing to boot on a bad import | [sprints/sprint-32.md](sprints/sprint-32.md) |
| 33 | Four page merges | **Built** 2026-09-05 — /compare became a slide-over panel on /players (`components/compare-panel.tsx` + a new `components/ui/slide-over.tsx`, lifted out of `MobileNav` where the app's only real drawer was inlined); /chips became the "Chip timing" tab on /transfers, taking the draft as a prop so one Squad selector governs both; /review moved under /team's gameweek selector, which was already a past-gameweek view; /status became the "Pipeline" tab on /settings. Each removed a duplicated loader or a second source of truth: /players and /compare fetched the same five tables and drew two horizon controls that couldn't agree, and /review's event picker could disagree with /team's. Two defects fell out — `toScoredPlayer` passed `pointsPerGame: null` only because the column wasn't selected, and folding /status behind /settings' sign-in wall would have taken a public page private (the auth gate is now scoped to the two tabs that need it). Four redirect stubs preserve `?ids=`/`?draft=`/`?event=` | [sprints/sprint-33.md](sprints/sprint-33.md) |
| 35 | Results-derived FDR | **Built and measured 2026-09-06 — the method works, and it still does not ship.** `lib/fdr-derived.ts` reconstructs both sides of every fixture from `player_gameweek_stats` (`fixtures` holds only 2026-27, so `deriveStandingsFromFixtures` cannot be walked over past seasons), rates opponents rolling and within-season shrunk toward the league mean, and bands to 1-5 by quintile. Improves MAE and r against the harness's neutral `fdr=3` in all four seasons, and a **shuffled control** — same rating values, team mapping destroyed — is worse than neutral everywhere, so the signal is real. **Not shipped:** the baseline is neutral, but production `fdrRun` uses FPL's *official* FDR, so derived-beats-official is untested. Also fixed a venue double-count that had roughly halved the effect | [sprints/sprint-35.md](sprints/sprint-35.md) |
| 34 | Post-GW3: accuracy scoreboard, 2026-27 in the model, Sprint 32 finished | **Built 2026-09-06** — GW3 finishing cleared the accuracy scoreboard's own stated blocker (≥2 gameweeks both archived *and* scored), so `lib/prediction-accuracy.ts` finally has a caller; a 1000-row cap bug in `loadArchivedEvents` would have silently halved its sample. 2026-27 added as a fourth walk-forward target plus a narrow `currentSeasonScope: "minutes"` blend — `scope=all` clears 2 of 4 seasons and `scope=minutes` 2 of 4 at w=0.3, **neither ships**. (The narrow scope's apparent 3-of-4 advantage came from the corrupted harness inputs this sprint also found, and does not survive the fix — it is untested, not supported.) Sprint 32's last eight functions deployed and its never-run verification gate executed, which caught `sync-manager` failing to boot | [sprints/sprint-34.md](sprints/sprint-34.md) |
| 36 | Historical decision analytics, a two-way Telegram bot, and the Todo sweep | **Shipped 2026-09-10.** All four phases built, merged and deployed the same day; every owner action closed except the CLI deploy, parked with DSI-55 — scoped from the Linear Todo column (six issues promoted out of Backlog that day), tracked as [DSI-74](https://linear.app/dsinha-org/issue/DSI-74). Four phases, each a stopping point. **Phase 1 built:** `lib/decision-analytics.ts` + a `/team` panel — captain success, transfer success over a user-set horizon, chip ROI, rank progression, no migration (`manager_chips` gets its first reader). The captain comparison moved out of `lib/gameweek-review.ts` rather than being duplicated, gained an *effective* (×multiplier−1) gap in place of a raw difference, and kept the starting XI as its benchmark — the scoped widening to all 15 was dropped, since an armband on a benched player earns nothing unless FPL subs them on. Verified against entry 274486 before the UI existed: GW3 reconstructs FPL's own 52 and its bench 18 exactly. **Phase 2 built:** `/team` rivals can be taken from a league's own table — one `addRivalById` path shared by the manual add and a capped, sequential batch, with a 429 handled as its own outcome that names which rivals landed. 8 of the owner's 13 leagues hold no synced standings, so the never-synced case gets a message naming the fix rather than an empty list; league 314 being among them independently corroborates DSI-60. **Phase 3 built and deployed:** Telegram in **both** directions — a cron-only `notify` over an outbox whose composed dedupe key is the feature, plus a `telegram-webhook` answering `/team`, `/points`, `/fixtures`, `/leagues` from already-computed rows only. Tables and Vault accessors applied and RLS verified across two authenticated users and anon (12 checks, including the unique index that makes two accounts claiming one chat unrepresentable); `@fpl_decision_bot` is live: both secrets held in Vault and never handled outside Postgres (the webhook secret was generated there and setWebhook is itself a database function), both functions deployed, the webhook registered with `allowed_updates` narrowed, and `notify` on a 15-minute schedule with its first `invoke_sync` run returning success. Verified negatively too: the webhook 401s a missing or wrong secret token, an unlinked chat reads nothing, and `notify` 401s the publishable key. **Phase 4 measured, and two of three notes turned out differently from what they implied:** the `verify_jwt` posture buys *no* access control (the publishable key is public, so both settings end at the same cron-secret check) — the real difference is an invocation spent before rejection, so the flip is cost and consistency, not hardening, and belongs in a CLI deploy rather than a 40KB inline re-upload of two cron functions; `scratch-path-test` is confirmed live, unauthenticated, and returns `hi` — open but leaking nothing, still an owner-only delete; and the 429's *counting* half is now proved in SQL (flips at exactly 30, window and per-function scoping correct), leaving only the HTTP render for a browser. League 314 confirmed at 0 rows. The three console/decision items (league 314 sampled and kept, custom SMTP, Sprint 32's deliberate gaps including a now-cheap proof of the 429). The two-way requirement came from DSI-65's Linear **comment**, not its description — the reason `/start-sprint` now reads comments | [sprints/sprint-36.md](sprints/sprint-36.md) |
...
| 39 | Player-profile bug sweep, prediction-pipeline reliability, expected-minutes cohort split | **Built and shipped 2026-09-21** — DSI-181 (7 Sprint-38 UI bugs, verified live in the preview browser), DSI-142 (`generate-predictions` 401s: bounded retry on a transient RPC error, `sync_runs` row written even on denial), DSI-178 (appeared/no-show + GKP cohort split and a start-probability Brier score added to `scripts/backtest-walkforward.ts`, run live against real data). DSI-55 reviewed alongside and rescoped, not built — see below | [sprints/sprint-39.md](sprints/sprint-39.md) |
| — | Swipe-to-dismiss gestures | **Built and shipped 2026-09-21** — `components/ui/slide-over.tsx` (the shared panel behind `MobileNav`, the mobile player card, and the compare panel) gained real `AnimatePresence` exit animations plus direction-aware swipe-to-dismiss: nav drags left, compare drags right, the player-card sheet drags down, each closing the way it opened. Added the `motion` dependency for this — the repo had no gesture library. A degenerate point `dragConstraints` (the textbook pattern) measurably misbehaved in this environment; shipped with a real-range constraint and an explicit spring-back instead. No new sprint number, prompted by the owner asking for the interface to feel more like a native mobile app | [sprints/swipe-to-dismiss.md](sprints/swipe-to-dismiss.md) |
| 40 | Latency — the shared bundle and the read waterfalls | **Built and shipped 2026-09-24** (DSI-184, PR #36). DSI-56: `next experimental-analyze` showed the engines were already per-route, so latency.md's hypothesis was wrong. The shared cost was `motion` in the nav's `SlideOver` (now lazy, prefetched on idle) and engines reached through the root layout's imports (moved to `lib/transfer-rules.ts`). Every route's first-load JS is down 135–153 KB. DSI-57: false read dependencies collapsed on `/players` (~3.2 → ~1.3 s), `/transfers` (~3.2 → ~2.2 s) and `/team` (~2.7 → ~2.2 s, rendered output byte-identical). DSI-77: dead lint suppressions removed, and the lint verdict made unambiguous. Found and not fixed: the price-watch scan downloads the whole 73k-row ownership table on every `/players`/`/transfers` visit | [sprints/sprint-40.md](sprints/sprint-40.md) |
| — | Search indexing (SEO) | **Built and shipped 2026-09-26** (PR #39) — `app/sitemap.ts` and `app/robots.ts` (both build-time static), and a title, description, canonical, Open Graph and Twitter tags on every indexable route, all driven by `SITE_ROUTES` in `lib/seo.ts`. Personal, auth and redirect-stub routes are `noindex`. A 1200×630 preview card in `public/og-image.png`. Sitemap submitted to Google Search Console (10 pages discovered) and imported into Bing. No new sprint number, prompted by the owner's Search Console submission | [sprints/seo.md](sprints/seo.md) |
...
**Sprint 40 shipped 2026-09-24** (PR #36). See [sprints/sprint-40.md](sprints/sprint-40.md).
Latency: the shared bundle (DSI-56), the read waterfalls (DSI-57), and lint (DSI-77). It left
three things for the next pick:
...
review: DSI-181 (7 Sprint-38 UI bugs — panel height, tooltip z-index, price-watch loading gap
on `/team`/`/deadline`/`/builder`, club-crest fallback, transfers-tab ownership, sortable
Price Watch column, and a price-proximity Telegram notification built on the flat watchlist
threshold rather than the fitted on-site reading, to keep the Deno boundary's "no modelled
quantity" rule intact), DSI-142 (`generate-predictions` 401s: a bounded retry on a transient
`verify_cron_secret` RPC error, and a `sync_runs` row written even when the cron secret is
rejected, so a refused run is no longer invisible to `/settings` → Pipeline), and DSI-178 (the
expected-minutes cohort-split backtest that replaced DSI-53, canceled the same day once its
precondition was answered "don't refit" — `scripts/backtest-walkforward.ts` now scores
appeared/no-show and GKP×appeared/no-show cohorts plus a start-probability Brier score, run
live against real data and showing the same cancelling pattern DSI-50 found). DSI-55 was
reviewed alongside it and found stale — the repo is already public and secret scanning/push
protection are already enabled — so it was rescoped to a single owner-check (fork-PR workflow
...
- **Sprint 32 — DONE. Built 2026-09-05, fully deployed and verified 2026-09-06.** All ten
  ...
  publishable key, including all five `?force=1` hatches (closed, not merely guarded —
  `verifyCron` runs before the URL is parsed); the browser-facing pair return their own 401s;
  cron kept succeeding across the deploy; and `anon` can read the Pipeline tab's columns but not
  ...
  **The repo flip is no longer blocked by anything technical — it is purely an owner decision.**

- **Page merges — four done 2026-09-05, three deliberately rejected.** The survey behind them
  is in [sprints/sprint-33.md](sprints/sprint-33.md) §5: `/scenarios` into `/builder`, `/news` into
  `/deadline` and `/leagues` into `/team` were all considered and turned down, so a future pass
  should read that section before re-proposing any of them. Shared state is not shared purpose.
- **A results-derived FDR — built and measured 2026-09-06 (Sprint 35); does not ship.** The signal
  is real: it improves MAE and Pearson r against the harness's neutral `fdr=3` in all four seasons,
  and a shuffled control (same rating values, team-to-rating mapping destroyed) is *worse* than
  neutral everywhere — so the gain comes from the ratings knowing which opponent is which, not from
  feeding `predict` a spread of numbers. Two real defects were caught by validating against live
  data first: a **venue double-count** in the banding (home and away ranked in one pool, so the 1-5
  mostly encoded venue, which `predict` already handles — fixing it roughly doubled the effect), and
  one-sided fixtures in the live season while `sync-player-history` catches up.
  **Why it does not ship:** the baseline is *neutral*, but production `ScoredPlayer.fdrRun` uses
  FPL's **official** FDR. Derived-beats-neutral is measured; derived-beats-official is not, because
  past-season official FDR is not obtainable. **The gate that would justify shipping** is
  derived-vs-official on 2026-27, where official FDR *is* available in `fixtures` — n=471 at GW3 is
  far too thin, so re-run from ~GW10. `priorGames` is deliberately not tuned from these numbers (2
  and 8 clear 4/4, 4 clears 3/4, all within 0.012 on the thinnest season — noise, not structure).
  See [sprints/sprint-35.md](sprints/sprint-35.md).
...
- **Blend current-season form into the xP model — built and measured 2026-08-27; does not clear
  ...
  **Attempt 3, four seasons and a narrow scope — swept 2026-09-06 (Sprint 34), does not clear the
  gate, not shipped.** GW3 finishing made 2026-27 a usable fourth walk-forward target, added to
  `targets` and reported as the weak season it is (three gameweeks: the last-5 baseline cannot fire
  below six, the blend arm has events 2-3 only, n=468). Alongside it, a narrower hypothesis:
  `ShrinkInput.currentSeasonScope` (`"all" | "minutes"`, default `"all"` so every earlier sweep and
  caller is untouched) lets the current season move `mpg`/`start_share` while holding the per-90
...
- **Archive pre-deadline predictions — built 2026-08-27.** `generate-predictions` deletes and
  ...
  `player_gameweek_stats` once a gameweek scores. The scoreboard panel built on it was
  deliberately deferred until ≥2 *scored* gameweeks existed, and **shipped 2026-09-06** on
  `/settings` → Pipeline once GW3 finished — see the Sprint 34 row above. RLS verified both roles; `accuracyStats` lifted out of the backtest script into
...
- **Sprint 10 (Ownership Intelligence), exact slice — built 2026-08-21.** The same deadline made any
  ...
  2000 rank-ordered entries, 30,000 picks, 0 failures — so it was never a data/engineering gap,
  just something nobody had run and kept for real (that test's rows were deleted afterward).
  **Run and kept 2026-09-10** (Sprint 36): 2,000 entries, 30,795 picks. See
...
| ~~Custom FDR~~ **strength-derived one display-only (Sprint 31); results-derived one built and measured (Sprint 35), still not shipped** | The strength-derived rating can never be backtested — `teams` holds one season and strength is a snapshot with no history. The results-derived one **is** backtestable and was: it beats a neutral `fdr=3` on MAE and r in all four seasons, with a shuffled control confirming the signal is real. It is not shipped because the measured comparison is against *neutral*, while production `fdrRun` uses FPL's **official** FDR — derived-vs-official needs the current season and ~GW10 of data. Note `fixtures` holds only 2026-27, so historical scorelines come from `player_gameweek_stats`, not `deriveStandingsFromFixtures` | [sprints/sprint-35.md](sprints/sprint-35.md) |
| ~~Accuracy scoreboard panel~~ **Unblocked and built 2026-09-06 (Sprint 34)** | GW3 being scored took the archive/results intersection from GW2 alone (n=1) to GW2 + GW3 (n=2), which is the condition this row named. Now rendered on `/settings` → Pipeline (`components/accuracy-scoreboard.tsx`) over the already-built `lib/prediction-accuracy.ts`, whose `loadArchivedEvents` had a real 1000-row-cap bug that would have reported GW2 only | [sprints/sprint-34.md](sprints/sprint-34.md) |
