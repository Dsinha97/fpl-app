...
| [sprints/sprint-38.md](sprints/sprint-38.md) | live | The full player profile (a three-tab modal, the same from every page), the price-watch reading fixed and then shown properly, and an owner-scoped shortlist. Carries three findings: `loadPriceProgress` had been reading ~2% of its data since Sprint 29 and looked quiet rather than broken; DSI-54's falls classifier passed its gate at tight budgets but is deliberately unwired; and a claim about xDC excluding forwards that turned out to be wrong, corrected in §4. |
