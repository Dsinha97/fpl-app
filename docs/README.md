...
| [sprints/sprint-32.md](sprints/sprint-32.md) | live | Locks down the `/functions/v1/` surface before the repo goes public. **Fully deployed and verified 2026-09-06** (Sprint 34): all ten functions gated, the verification matrix run for the first time, and one real regression caught by it (`sync-manager` failing to boot on a bad import). §5c has the results and what remains open. |
| [sprints/sprint-35.md](sprints/sprint-35.md) | live | A results-derived FDR — backtestable where Sprint 31's strength-derived one never could be. Beats a neutral baseline in all four seasons with a shuffled control confirming the signal is real; still not shipped, because the baseline is neutral while production uses FPL's official FDR. |
| [sprints/sprint-34.md](sprints/sprint-34.md) | live | Post-GW3 — the model-accuracy scoreboard (its ≥2-archived-and-scored blocker cleared when GW3 was scored), 2026-27 added to the walk-forward backtest plus a narrow minutes-only blend scope (3 of 4 seasons, still not shippable), and Sprint 32 finished: last eight functions deployed, its verification gate run for the first time. Also fixes two real harness defects — unordered `limit`/`offset` paging that was duplicating ~26% of rows, and position resolved from today's roster — which together restore the published backtest tables and make a run deterministic. |
...
| [sprints/m9.md](sprints/m9.md) | live | The design audit, in four sprints — primitives built, adopted app-wide, per-screen passes, then the feature-shaped items. Records what the audit got wrong as carefully as what it got right (roughly half of each item was already fixed, already false, or would have been a regression), the two pieces of tooling the sprint had to build first, and the two items left deliberately open. |
| [sprints/sprint-36.md](sprints/sprint-36.md) | live | Historical decision analytics on `/team`, rivals addable from a league's standings, and a two-way Telegram bot (`notify` + `telegram-webhook`). Also the three pipeline faults found along the way, all the same shape — work that silently does not happen and renders as absence. |
| [sprints/sprint-38.md](sprints/sprint-38.md) | live | The full player profile (a three-tab modal, the same from every page), the price-watch reading fixed and then shown properly, and an owner-scoped shortlist. Carries three findings: `loadPriceProgress` had been reading ~2% of its data since Sprint 29 and looked quiet rather than broken; DSI-54's falls classifier failed its gate once a counter-reset bug in its feature was fixed (an earlier run of the same gate appeared to pass); and a claim about xDC excluding forwards that turned out to be wrong, corrected in §4. |
| [sprints/sprint-37.md](sprints/sprint-37.md) | historical | The M9 design-audit Todo sweep — 13 independent follow-ups in three phases (interface/a11y, writing, animation). One fix landed on a component Sprint 28 had already removed from every route: correct, and currently unreachable. |
| [sprints/gw5-check-in.md](sprints/gw5-check-in.md) | live | **The M6 GW5 check-in (2026-09-20), and the coverage bug it found first.** The accuracy scoreboard was counting a half-synced gameweek's unwritten fixtures as real blank returns. The check-in itself: the bias sign held but collapsed to −0.084, and decomposes into two large opposite halves — so the GW10 `positionCalibration` refit was written out of the plan. Also DSI-54's baseline, and a batch of `/deadline`/`/team` interface work. |
| [sprints/swipe-to-dismiss.md](sprints/swipe-to-dismiss.md) | live | Swipe-to-dismiss gestures for the nav menu, player card, and compare panel — one change to the shared `SlideOver` primitive, direction-derived from its existing `side` prop. Added the `motion` dependency (the repo had none); a degenerate point `dragConstraints` measurably misbehaved in this environment, so it shipped with a real-range constraint and an explicit spring-back instead. |
| [sprints/seo.md](sprints/seo.md) | live | Search indexing: sitemap, robots.txt, per-route titles, canonicals and preview tags from one route list (`lib/seo.ts`), `noindex` on personal and redirect routes, a static preview card. Why the `opengraph-image` file convention was dropped, and the Search Console, Bing and Cloudflare steps taken outside the repo. |
